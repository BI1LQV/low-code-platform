...
watchEffect(() => {
  if (funcStore.requirePyodide && !loaded.value) {
    if (pyodideLoading["加载Pyodide"]) {
      return
    }
    setRefToReactive(pyodideLoading, "加载Pyodide", usePromiseStatus(
      load())[0])
  }
})
...
<template>
  <Teleport to="body">
    <Transition name="loading-page">
      <div v-if="showLoading" class="flex flex-col justify-center items-center text-24px text-white w-full h-full bg-[rgba(0,0,0,0.7)] absolute left-0 top-0 z-2000">
